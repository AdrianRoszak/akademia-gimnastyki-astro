...
} from './astro-designed-error-pages_CVeI_OOX.mjs'
import {
  A as AstroError,
  ...
  W as slash$1,
  B as trimSlashes,
} from './astro/assets-service_hu8gJn31.mjs'
import {
  A as ASTRO_VERSION,
  ...
  y as yellow,
} from './astro/server_BzgV5NAo.mjs'
...
function requestHasLocale(locales) {
  return (context) => pathHasLocale(context.url.pathname, locales)
}
...
function redirectToFallback({ fallback, locales, defaultLocale, strategy, base }) {
  return (context, response) => {
...
class AstroCookies {
  ...
  delete(key, options) {
    ...
    this.#ensureOutgoingMap().set(key, [DELETED_VALUE, serialize_1(key, DELETED_VALUE, serializeOptions), false])
  }
  /**
  ...
  set(key, value, options) {
    ...
    this.#ensureOutgoingMap().set(key, [serializedValue, serialize_1(key, serializedValue, serializeOptions), true])
  ...
  *headers() {
  ...
  #parse(options = void 0) {
    ...
    this.#requestValues = parse_1$1(raw, options)
  }
}
        ...
        if (i < length - 1) {
          if (payload) {
            ...
            if (payload instanceof Request) {
              ...
              newRequest = new Request(new URL(payload, handleContext.url.origin), handleContext.request)
            }
...
function generatePaginateFunction(routeMatch) {
  return function paginateUtility(data, args = {}) {
    ...
    const result = [...Array(lastPage).keys()].map((num) => {
      ...
      const next =
        pageNum === lastPage ? void 0 : correctIndexRoute(routeMatch.generate({ ...params, page: String(pageNum + 1) }))
      const prev =
        pageNum === 1
          ? void 0
          : correctIndexRoute(
...
class RenderContext {
   ...
   */
  counter = 0
  static create({ locals = {}, middleware, pathname, pipeline, request, routeData, status = 200, props }) {
  ...
  async render(componentInstance, slots = {}) {
    ...
    const lastNext = async (ctx, payload) => {
      if (payload) {
        ...
        const [routeData, component] = await pipeline.tryRewrite(payload, this.request, this.originalRoute)
        this.routeData = routeData
  ...
  computeCurrentLocale() {
    ...
    return (this.#currentLocale ??=
      computeCurrentLocale(routeData.route, locales) ?? computeCurrentLocale(url.pathname, locales) ?? fallbackTo)
  }
  #preferredLocale
...
class App {
  ...
  #createPipeline(manifestData, streaming = false) {
    if (this.#manifest.checkOrigin) {
      this.#manifest.middleware = sequence(createOriginCheckMiddleware(), this.#manifest.middleware)
    }
...
path$7.convertPosixPathToPattern =
  path$7.convertWindowsPathToPattern =
  path$7.convertPathToPattern =
  path$7.escapePosixPath =
  path$7.escapeWindowsPath =
  path$7.escape =
  path$7.removeLeadingDotSegment =
  path$7.makeAbsolute =
  path$7.unixify =
    void 0
const os = require$$1
...
class Matcher {
  ...
  _splitSegmentsIntoSections(segments) {
    return utils$5.array.splitWhen(
      segments,
      (segment) => segment.dynamic && utils$5.pattern.hasGlobStar(segment.pattern),
    )
  }
}
...
;((exports) => {
  ...
  class Settings {
    constructor(_options = {}) {
      ...
      this.deep = this._getValue(this._options.deep, Number.POSITIVE_INFINITY)
